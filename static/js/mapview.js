...
function loadResponseDataCallback()
{
    ...
    if(navContainer.length == 1)
    {
        ...
        if(formJSONMngr.getNumSelectOneQuestions() > 0)
        {
            ...
            dropdownCaretLink = _createElementAndSetAttrs('a', {"href":"#", "class":"dropdown-toggle",
                "data-toggle":"dropdown"});
            dropdownCaret = _createElementAndSetAttrs('span', {"class":"view-by-link"}, gettext("View By"));
            dropdownCaretIcon = _createElementAndSetAttrs('i', {"class":"fa fa-caret-right"});
            dropdownCaret.appendChild(dropdownCaretIcon);
            dropdownCaretLink.appendChild(dropdownCaret);
            dropDownContainer.appendChild(dropdownCaretLink);

            var questionUlContainer = _createElementAndSetAttrs("ul", {"class":"dropdown-menu-right dropdown-menu questions"});
...
function get_random_color(step, numOfSteps) {
    // This function generates vibrant, "evenly spaced" colours (i.e. no clustering). This is ideal for creating easily distiguishable vibrant markers in Google Maps and other onadata.apps.
